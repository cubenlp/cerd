# This code is based on the revised code from fastchat based on tatsu-lab/stanford_alpaca and QwenLM/Qwen1.5.
...
class ModelArguments:
    base_model_name_or_path: Optional[str] = field(default="Qwen/Qwen-7B")


@dataclass
class DataArguments:
    training_set_path: str = field(
        default=None, metadata={"help": "Path to the training data."}
    )
    validation_set_path: str = field(
...
def make_supervised_data_module(
    ...
    train_data = []
    with open(data_args.training_set_path, "r") as f:
        for line in f:
            train_data.append(json.loads(line))
    train_dataset = dataset_cls(train_data, tokenizer=tokenizer, max_len=max_len)

    if data_args.validation_set_path:
        eval_data = []
        with open(data_args.validation_set_path, "r") as f:
            for line in f:
